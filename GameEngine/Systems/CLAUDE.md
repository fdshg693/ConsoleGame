...
```
Systems/
  GameSystem.cs         # ステップ駆動エンジン（Start / Step）＋コンソール用駆動ループ
...
### EventManager
- コンストラクタ: `EventManager(IPlayer player, GameConfig config, IEnemyFactory enemyFactory, Random? random = null)`（描画・入力に非依存。`random` 既定は `new Random()`）
- 内部で `new BattleManager(player, enemyFactory)` を生成して戦闘進行を委譲する
...
### BattleManager
- コンストラクタ: `BattleManager(IPlayer player, IEnemyFactory enemyFactory)`（描画・入力に非依存）
