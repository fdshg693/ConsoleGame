...
```
Program.cs (エントリポイント)
  -> GameConfigLoader (YAML設定読み込み)
  -> Player生成 (ExperienceManager, InventoryManager を注入)
...
- `GameConfigLoader.Instance` で設定を事前読み込み（起動時バリデーション）
- プレイヤー名をコンソール入力（空の場合は "Hero"）
- `CreatePlayer()` で `ExperienceManager`, `InventoryManager` を注入して `Player` を生成
- `GameSystem.RunGameLoop()` でメインループ開始
